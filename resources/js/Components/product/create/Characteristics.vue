...
<template>
    ...
    <div v-for="(characteristic, index) in !props.isEdit ? useCreateProductStore().characteristics : useEditProductStore().characteristics" :key="index" class="flex items-center gap-x-2.5 mt-2.5 ">
        <input maxlength="255" v-model="characteristic.characteristic_key" type="text" class="p-2.25 w-57.5 h-10 border-2 border-[#2980B9] rounded-[10px] focus:outline-none max-[510px]:w-full" :placeholder="useTranslateStore().t('size')">
        <div class="min-w-5 h-0.5 bg-white"></div>
        <input maxlength="255" v-model="characteristic.characteristic_value" type="text" class="p-2.25 w-57.5 h-10 border-2 border-violet-800 rounded-[10px] focus:outline-none max-[510px]:w-full" :placeholder="useTranslateStore().t('24sm')">
