...
import TopNotification from '../../../Widgets/Toasts/TopNotification.vue'
...
const changeName = async () => {
    ...
    try{
        const res = await axios.patch(route('user.change.name'), {name: userData.name, lang: useTranslateStore().currentLang});
        setTimeout(() => {
...
<template>
    ...
    <div class="flex items-center gap-x-2.5 mt-2.5">
        ...
        <div v-if="btnsChange.email" class="flex gap-x-2.5 items-center">
            <input v-model="userData.email" type="text" class="p-2.25 w-70 h-7.5 border-2 border-lime-500 rounded-[10px] focus:outline-none" :placeholder="useTranslateStore().t('enterEmail')">
            <img class="cursor-pointer rounded-[5px] hover:shadow-[0_0px_15px_0_rgba(138,201,121,1)] transition duration-150" src="/public/img/confirm.svg" alt="confirm">
            <img @click.prevent="btnsChange.email = !btnsChange.email, userData.email = useUserStore().email" class="cursor-pointer rounded-[5px] hover:shadow-[0_0px_15px_0_rgba(255,0,0,1)] transition duration-150" src="/public/img/cancel.svg" alt="cancel">
        </div>
    </div>
    <div class="flex items-center gap-x-2.5 mt-2.5">
        ...
        <button class="border text-[14px] h-10 p-2.5 flex justify-center items-center border-red-500 bg-red-500 rounded-[10px] hover:bg-inherit hover:text-red-500 transition duration-300 cursor-pointer">{{ useTranslateStore().t('logout') }}</button>
    </div>
    <div class="flex items-center gap-x-2.5 mt-2.5">
