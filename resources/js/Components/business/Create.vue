...
<template>
    ...
    <form @submit.prevent="createBrand" class="flex items-center gap-x-2.5">
        <input v-model="brandName" :readonly="load.createBrand" type="text" maxlength="255" class="p-2.25 w-75 h-10 border-2 border-lime-500 rounded-[10px] focus:outline-none max-[425px]:w-full" :placeholder="useTranslateStore().t('enterBrand')" required>
        <button v-if="!load.createBrand" type="submit" class="btn-blue h-10 w-20">{{ useTranslateStore().t('create') }}</button>
        <div v-if="load.createBrand" class="min-w-7.5 min-h-7.5 border-3 text-blue-400 text-4xl animate-spin border-gray-300 flex items-center justify-center border-t-blue-400 rounded-full"></div>
    </form>
